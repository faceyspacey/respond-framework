...
export default ({ state, respond }) => {
  ...
  replayState.settings ??= nestAllSettings(state.configs, state.settings) // replayState.settings is generated by tapping reload, eg while focusing a particular module; it will then be saved to test files and brought back when those tests are played or via sessionStorage on refreshes; however on first visit, it won't exist, so we need to generate the full settings tree with default settings, as if the top level module is the focusedBranch
