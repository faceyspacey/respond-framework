...
export default ({ state, respond }) => {
  ...
  Object.assign(state, createConfigsAndSettings(top, branches, depth, replayState))
  replayState.settings ??= nestAllSettings(state.settings) // replayState.settings is generated by tapping reload, eg while focusing a particular module; it will then be saved to test files and brought back when those tests are played or via sessionStorage on refreshes; however on first visit, it won't exist, so we need to generate the full settings tree with default settings, as if the top level module is the focusedBranch
...
const createConfigsAndSettings = (top, branches, depth, replayState) => {
  const { branch: focusedBranch, settings: input } = replayState

  const configs = {}
  const settings = {}

  const branch = input?.dependedBranch ?? focusedBranch // settings with a module start higher than the branch where they get their ancestor replays from
