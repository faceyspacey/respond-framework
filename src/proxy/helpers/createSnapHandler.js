...
import { canProxy, getProto, getOpd } from './utils.js'
import { _module, _parent } from '../../createModule/reserved.js'
...
const getModule = (state, protoDescriptors) => (snap, k, proxy) => {
  ...
  if (k === 'prevState') {
    const topSnapPrevState = useSnapshotPrevState(snap.respond.topState) // need to treverse from top to facilitate props reactivity from parents which possibly gets passed from top
