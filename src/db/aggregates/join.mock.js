...
export const joinSum = async (db, models, specs) => {
  for (const s of specs) {
    ...
    for (const m of models) {
      ...
      const joinedModels = await collection.findMany({ ...range, ...$match })   // user.postCount = eg: db.post.count({ userId: user.id })
...
export const joinInner = async (db, models, specs, selector) => {
  ...
  for (const s of specs) {
    ...
    for (const m of models) {
      ...
      const count = await collection.count(selector)                           // eg: db.post.count({ userId: user.id, category: 'tech' }) -> will filter user out if no tech posts
