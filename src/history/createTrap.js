...
export const popListener = async () => {
  ...
  delete bs.pop                                       // ...so changedUrl is queued, so we can go to tail if no change made, OR use replaceState as browsers don't honor history stack when more than one push is performed per user-triggered event
